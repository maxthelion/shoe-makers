 ...
 */
export function computeProcessPatterns(actions: string[]): { reactiveRatio: number; reviewLoopCount: number } {
  let reactiveTicks = 0;
  ...
  return { reactiveRatio, reviewLoopCount };
}
 ...
 */
export async function getShiftProcessPatterns(repoRoot: string): Promise<{ reactiveRatio: number; reviewLoopCount: number } | undefined> {
