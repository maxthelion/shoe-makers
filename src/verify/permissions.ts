...
function buildRoleMap(wikiDir: string): Record<ActionType, RolePermissions> {
  ...
  return {
    ...
    "continue-work": {
      role: "executor",
      canWrite: ["src/", wikiPath, ".shoe-makers/state/", ".shoe-makers/claim-evidence.yaml", ".shoe-makers/config.yaml", "CHANGELOG.md", "README.md"],
    ...
    "execute-work-item": {
      role: "executor",
      canWrite: ["src/", wikiPath, ".shoe-makers/state/", ".shoe-makers/claim-evidence.yaml", ".shoe-makers/config.yaml", "CHANGELOG.md", "README.md", "package.json", "bun.lock", "bun.lockb"],
      cannotWrite: [...ALWAYS_FORBIDDEN],
    },
    "dead-code": {
